...
CREATE TABLE IF NOT EXISTS `driver_licenses` (
    ...
    CONSTRAINT ck_driver_licenses_type CHECK (
        type IN ('CLASS1', 'CLASS2', 'HGV')
    )
...
CREATE TABLE IF NOT EXISTS `vehicles` (
    ...
    CONSTRAINT ck_vehicles_fuel CHECK (
        fuel IN (
            'GASOLINE',
            'LPG',
            'ELECTRIC',
...
CREATE TABLE IF NOT EXISTS `allowance_types` (
    ...
    CONSTRAINT ck_allowance_types_status CHECK (
        status IN ('ACTIVE', 'DELETED')
    )
) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `deduction_types` (
    ...
    CONSTRAINT ck_deduction_types_status CHECK (
        status IN ('ACTIVE', 'DELETED')
    )
) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `driver_payrolls` (
    ...
    CONSTRAINT ck_driver_payrolls_period_range CHECK (
...
CREATE TABLE IF NOT EXISTS `delete_logs` (
    ...
    CONSTRAINT uq_delete_logs UNIQUE (
        table_name,
        record_id,
        delete_type
    ),
    CONSTRAINT fk_delete_logs_deleted_by FOREIGN KEY (deleted_by) REFERENCES users (id) ON DELETE SET NULL,
    CONSTRAINT ck_delete_logs_delete_type CHECK (
        delete_type IN ('SOFT', 'HARD')
    )
) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `users_role_logs` (
    ...
    CONSTRAINT ck_users_role_logs_prev_role CHECK (
        prev_role IN (
            'ADMIN',
            'DRIVER',
            'CUSTOMER',
    ...
    CONSTRAINT ck_users_role_logs_new_role CHECK (
        new_role IN (
            'ADMIN',
            'DRIVER',
            'CUSTOMER',
...
CREATE TABLE IF NOT EXISTS `customers_status_logs` (
    ...
    CONSTRAINT ck_customers_status_logs_prev_status CHECK (
        prev_status IN (
            'PENDING',
            'APPROVED',
            'REJECTED'
...
CREATE TABLE IF NOT EXISTS `contracts_status_logs` (
    ...
    CONSTRAINT ck_contracts_status_logs_prev_status CHECK (
        prev_status IN (
            'PENDING',
            'APPROVED',
            'REJECTED',
    ...
    CONSTRAINT ck_contracts_status_logs_new_status CHECK (
        new_status IN (
            'PENDING',
            'APPROVED',
            'REJECTED',
...
CREATE TABLE IF NOT EXISTS `vehicles_status_logs` (
    ...
    CONSTRAINT ck_vehicles_status_logs_prev_status CHECK (
        prev_status IN (
            'AVAILABLE',
            'IN_USE',
            'UNDER_MAINTENANCE',
    ...
    CONSTRAINT ck_vehicles_status_logs_new_status CHECK (
        new_status IN (
            'AVAILABLE',
            'IN_USE',
            'UNDER_MAINTENANCE',
...
CREATE TABLE IF NOT EXISTS `deliveries_status_logs` (
    ...
    CONSTRAINT ck_deliveries_status_logs_prev_status CHECK (
        prev_status IN (
            'REQUESTED',
            'RECEIPTED',
            'CANCELLED',
...
CREATE TABLE IF NOT EXISTS `allocations_status_logs` (
    ...
    CONSTRAINT ck_allocations_status_logs_prev_status CHECK (
        prev_status IN (
            'ASSIGNED',
            'IN_PROGRESS',
            'COMPLETED',
    ...
    CONSTRAINT ck_allocations_status_logs_new_status CHECK (
        new_status IN (
            'ASSIGNED',
            'IN_PROGRESS',
            'COMPLETED',
...
CREATE TABLE IF NOT EXISTS `driver_payrolls_status_logs` (
    ...
    CONSTRAINT ck_driver_payrolls_status_logs_prev_status CHECK (
        prev_status IN (
            'CREATED',
            'CONFIRMED',
            'DELETED'
    ...
    CONSTRAINT ck_driver_payrolls_status_logs_new_status CHECK (
        new_status IN (
            'CREATED',
            'CONFIRMED',
            'DELETED'
